...
def train_model(x_train, y_train, x_test, y_test):
    ...
    # Initial val_loss is equal to infinity
    ...
    # If the user press CTRL+C, the program won't exit ; it will instead stop the current training.
...
if __name__ == '__main__':
    ...
    # Override with your own loading method
    x_train, y_train, x_test, y_test = load_data()

    # Only useful if the vector is a one-hot vector
    x_train = sequence.pad_sequences(x_train)
    x_test = sequence.pad_sequences(x_test)
    y_train = to_categorical(y_train, get_auth_number())
    y_test = to_categorical(y_test, get_auth_number())

    # Set the log directory to the format CNN_MaxF([0-9]+)_BS([0-9]+)_LenAlph([0-9]+)
    log_dir = './logs/CNN_MaxF' + str(max_features) + '_BS' + str(batch_size) + '_LenAlph' + str(len(alphabet))
    element_in_dir = []

    # Create a directory according to the path log_dir + '/1'. If log_dir already exists, 
    # given n the number of subdirectory in the directory, it will create a folder to the path log_dir + '/' + str(n)
